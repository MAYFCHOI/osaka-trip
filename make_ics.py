...
lines = [
    ...
    "METHOD:PUBLISH", "X-WR-CALNAME:大阪 二泊三日", "X-WR-TIMEZONE:Asia/Tokyo",
...
for day in data["days"]:
    ...
    for i, it in enumerate(items):
        ...
        for t, title in (it.get("ics") or [[it["t"], it["name"]]]):
            events.append((t, title, it, nxt))
    for t, title, it, nxt in events:
        start = t.replace(":", "") + "00"
        # 종료 = 다음 일정 시작(최대 3시간), 없으면 +30분
        ...
        if nxt:
            ...
            dur = 30
        em_total = sh * 60 + sm + dur
        end = f"{em_total // 60:02d}{em_total % 60:02d}00"
        ...
        ev = [
            ...
            f"DTSTAMP:{stamp}",
            f"DTSTART;TZID=Asia/Tokyo:{d}T{start}",
            f"DTEND;TZID=Asia/Tokyo:{d}T{end}",
            f"SUMMARY:{esc(title)}",
        ]
        if desc: ev.append(f"DESCRIPTION:{esc(desc)}")
        url = map_url(it)
        if url:
            ev.append(f"LOCATION:{esc(it.get('q') or it['name'])}")
            ev.append(f"URL:{url}")
        if it.get("lat") is not None: ev.append(f"GEO:{it['lat']};{it['lng']}")
